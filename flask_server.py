...
def get_stat():
    ...
    if (request.args.get("rdate") is None):
        ...
        if offset is None:
            ...
            print(offset)
            if offset >= 0:
                date = datetime.datetime.strptime(request.args.get("rdate") + " 23:59:59-"+offset_str+":00", "%Y-%m-%d %H:%M:%S%z")
            else:
                date = datetime.datetime.strptime(request.args.get("rdate") + " 23:59:59+" + offset_str + ":00",
                                                  "%Y-%m-%d %H:%M:%S%z")
